...
### 路径 A：Hook + Telegram Bot API（轻量级）

直接在 Hook 脚本里调 Telegram Bot API 发消息。无需 Channel 机制。

**适用场景**: 单向通知，不需要从 Telegram 回复控制 CC

```
Hook 事件 → Shell 脚本 → curl Telegram Bot API → 用户收到消息
```
...
**配置**:
- `TG_BOT_TOKEN`: Telegram Bot Token（环境变量）
- `TG_CHAT_ID`: 接收通知的 Chat ID（环境变量）
- 存放在 `~/.auto-claude/config.env`

### 路径 B：Channel 双向通信（完整版）

利用 CC 的 Channel 机制，基于 MCP Server 实现双向 Telegram 通信。

**适用场景**: 不仅要收通知，还想从 Telegram 发指令给 CC

```
CC 事件 → Channel MCP Server → Telegram Bot API → 用户
用户回复 → Telegram Webhook → Channel MCP Server → 注入 CC Session
```

**与官方 Telegram Plugin 的关系**:

CC 已有官方 `telegram@claude-plugins-official` 插件（已在 settings.json 中启用）。
我们的方案：
- **优先复用官方插件** 的通信通道
- 在其基础上增加 Hook → Channel 的桥接
- 如果官方插件不满足需求，才自建 Channel

**自建 Channel 的能力**:
- 推送通知（单向）
- 接收指令（双向）
- 权限中继（远程审批 CC 的工具调用请求）
- 自定义消息格式（Markdown、按钮等）

### 推荐方案

**先实现路径 A（Hook + Bot API）**，因为：
1. 不依赖额外的 MCP Server 进程
2. 配置简单，开箱即用
3. 覆盖 90% 的通知需求
...
│   ├── teammate-idle.sh             # 队友空闲检测
│   └── notify.sh                    # Telegram 通知工具函数
│
├── config/                          # 配置模板
...
│
├── channel/                         # Channel MCP Server（路径 B，后期）
│   ├── src/
    ...
    "TeammateIdle": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "检查这个 teammate 是否完成了分配的任务。如果还有未完成的工作，返回 {\"ok\": false, \"reason\": \"描述未完成的工作\"}。如果确实完成了，返回 {\"ok\": true}。"
          }
        ]
      }
    ],
    "StopFailure": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "${CLAUDE_PROJECT_DIR}/auto-claude/hooks/notify.sh error"
          }
        ]
      }
    ]
...
### Phase 2: Telegram 通知（路径 A）
1. 通知工具函数（notify.sh）
2. 集成到 Stop Hook（完成/出错时通知）
3. 集成到 StopFailure Hook（错误通知）
4. 配置模板

### Phase 3: 双向控制（路径 B，按需）
1. Channel MCP Server 骨架
2. 桥接 Hook 事件到 Channel
3. 从 Telegram 接收指令
4. 权限中继
