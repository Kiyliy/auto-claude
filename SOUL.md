...
**事件**: `Stop`
**类型**: `command` (shell 脚本)
**运行模式**: 自动检测，支持 Agent Team / Subagent / 普通三种模式

**信号源（同时检查，不互斥）**:

Stop Hook 同时收集三个信号源来判断是否需要续命：

| 信号 | 来源 | 说明 |
|------|------|------|
| `active_subagents` | `~/.auto-claude/state/{sid}.json`（state.sh 手动计数） | SubagentStart/Stop hook 维护 |
| `active_teammates` | `~/.claude/teams/{teamName}/config.json`（CC 原生） | `members.length - 1`，teammate 退出时自动移除 |
| `pending_tasks` | `~/.claude/tasks/{teamName}/*.json`（CC 原生） | `status != "completed"` 的 task 数量 |

Agent Team 检测方法：遍历 `~/.claude/teams/*/config.json`，查找 `leadSessionId == session_id` 的 team。
...
处理流程:
  1. stop_hook_active 连续 block 计数
     - 累加计数，达到 MAX_CONSECUTIVE_BLOCKS 上限则强制放行
     - 未达上限则继续判断

  2. 同时收集所有信号（不是 if/else 互斥）
     - active_subagents ← state.sh 计数
     - detected_team ← 遍历 ~/.claude/teams/
     - active_teammates ← team config.json members
     - pending_tasks ← task 目录 status 统计

  3. 有活跃 agent（subagents > 0 OR teammates > 0）→ 放行
     agent 还在干活，lead 停下来是正常的

  4. 无活跃 agent + 有 team + 全部信号 == 0 → 放行 + 通知
     真正完成

  5. 无活跃 agent + pending_tasks > 0 → 续命
     还有未完成 task，需要 lead 继续分配

  6. 无活跃 agent + 非 team 模式 → 续命
     普通续命逻辑

  7. 续命前检查 continuation_count >= MAX_CONTINUATIONS → 放行 + 通知
     否则 → Block + 注入续命指令
...
输出（Block 时）:
  {
    ...
    "hookSpecificOutput": {
      "hookEventName": "Stop",
      "additionalContext": "<根据上下文生成的续命指令>"
    }
  }
```
...
统一使用 `prompts/stop-continue.md` 模板，支持 `{{count}}` 和 `{{max}}` 占位符。
...
**自有状态** — `~/.auto-claude/state/{session_id}.json`（通过 `lib/state.sh` 管理）:
...
**CC 原生状态**（只读，Agent Team 模式使用）:

| 路径 | 格式 | 用途 |
|------|------|------|
| `~/.claude/teams/{teamName}/config.json` | `{leadSessionId, members: [...]}` | 检测 team lead + 活跃 teammate 数 |
| `~/.claude/tasks/{teamName}/*.json` | `{status: "pending"\|"in_progress"\|"completed"}` | 统计未完成 task 数 |

CC 自动维护这些文件，teammate 退出时从 `members` 移除，task 完成时更新 `status`。Stop Hook 只读取不写入。
